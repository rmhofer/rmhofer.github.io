...
__author__ = 'Matthias Hofer'
__author_email__ = 'mhofer@mit.edu'
...
if False:
    new_files = []
    for f in files:
        base_name, extension = splitext(f)
        name_parts = base_name.split('-')
        new_filename = f"{name_parts[0]}{name_parts[2]}-{name_parts[1]}{extension}"
        rename(join(path, f),join(path, new_filename))
...
input_gallery['pictures'] = sorted(input_gallery['pictures'], key=lambda k: k['filename'])

# write to output file
print('Writing YAML data to file...')
with open(output_file, 'w') as f:
    f.write( yaml.dump(input_gallery, default_flow_style=False, sort_keys=True) )